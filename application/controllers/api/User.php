...
class User extends RestController
{
    ...
    public function dashboard_get()
    {
        ...
        foreach ($data->result_array() as $items) {
            if ($items['type'] === 'hadir' || $items['type'] === 'Hadir') {
                $hadir++;
            } else if ($items['type'] === 'izin' || $items['type'] === 'Izin') {
                $izin++;
            } else if ($items['type'] === 'alpha' || $items['type'] === 'Alpha') {
                $alpha++;
            }
    ...
    public function editMatkul_post()
    {
        ...
        $matkul = $this->m_user->editMatkul_api($data, $idMatkul);
        if ($matkul) {
            $this->response([
                'status' => 'success',
                'msg' => 'Submit successfully',
    ...
    private function kondisiHari($hari)
    {
        $noHari = 0;
        if ($hari === "Senin" || $hari === "senin") {
            $noHari = 1;
        } else if ($hari === "Selasa" || $hari === "elasa") {
            $noHari = 2;
